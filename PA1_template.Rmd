...
```{r totalnumberperdaywithoutna, fig.height=4}
...
Its mean is $`r mean_withoutna`$ and its median is $`r median_withoutna`$.  
...
```{r dailyactivitywithoutna, fig.height=4}
...
Interval $`r n[max_interval]`$ contains the maximum average number of steps $`r avgperinterval[max_interval]`$. 
...
The dataset contains `r nummissingvals` missing values in the steps column.
...
```{r totalnumberperdaywithna, fig.height=4}
...
Its mean is $`r mean_withna`$ and its median is $`r median_withna`$. By imputing the missing values, the mean has changed by $`r mean_withna-mean_withoutna`$ and the median by $`r median_withna-median_withoutna`$. It appears that the median is more robust towards imputing values than the mean for the chosen filling strategy.
...
```{r weekdayfactoractivity, fig.height=4, fig.width=10}
